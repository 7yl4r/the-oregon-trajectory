...
Game = (function() {
  ...
  Game.prototype._getRemainingLocations = function() {
    var j, len, location, ref, remainingLocs;
    remainingLocs = [];
    ref = this.locations;
    for (j = 0, len = ref.length; j < len; j++) {
      location = ref[j];
      if (location.x > this.distanceTraveled) {
  ...
  Game.prototype._getStatsToNextLocation = function() {
    ...
    for (i in remaining) {
      if (remaining[i].x < next.distance) {
...
},{"angular":29,"game":9,"game-btn":10,"howler":2,"ng-hold":15,"nodule":4}],17:[function(require,module,exports){
require('angular');
  ...
  'data', '$scope', '$rootScope', function(data, $scope, $rootScope) {
    ...
    this.getRandomName = (function(_this) {
      return function() {
        ...
        return _this.stationName = names[Math.floor(Math.random() * names.length)];
      };
    })(this);
